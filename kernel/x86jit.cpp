...
bool X86JIT::handlePageFault(Hart *hart, InterruptFrame *frame, bool isWrite)
{
	...
	if (ip < uintptr_t(codeRegionStart) || ip >= uintptr_t(codeRegionCurrent))
		return false; // Not caused by JIT generated code

	this->jitScause = isWrite ? Hart::SCAUSE_STORE_PAGE_FAULT : Hart::SCAUSE_LOAD_PAGE_FAULT;

	// Return from the call in jumpToCode
...
{
	if (rvReg == 0)
		return; // x0 is zeroed on the next read instead
...
{
	// emitFlushRVReg does not actually flush x0, so dirty must remain as-is
	if (rvReg == 0)
		return;
...
bool X86JIT::translateRVCInstruction(PhysAddr addr, uint16_t inst)
{
	if ((inst & 0xE003) == 0xA001) { // c.j
		...
		emitREX(true, false, false, regREXBit(rdX86));
		emit8(0xC1);
		emit8(0xC0 | (5 << 3) | regLow3Bits(rdX86));
		emitRaw<int8_t>(imm);
		return true;
		...
		emitREX(true, false, false, regREXBit(rdX86));
		emit8(0xC1);
		emit8(0xC0 | (7 << 3) | regLow3Bits(rdX86));
		emitRaw<int8_t>(imm);
		return true;
		...
		return true;
	} else if ((inst & 0b111'1'11'000'10'000'11) == 0b100'1'11'000'00'000'01) { // c.addw/c.subw
		bool isAdd = inst & (1 << 5);
		...
		// add/sub %rs2X86d, %rdX86d
		emitREX(false, regREXBit(rs2X86), false, regREXBit(rdX86));
		emit8(isAdd ? 0x01 : 0x29);
		emit8(0xC0 | (regLow3Bits(rs2X86) << 3) | regLow3Bits(rdX86));
...
bool X86JIT::translateInstruction(PhysAddr addr, uint32_t inst)
{
	...
	case 0x53u: // FP
	{
		...
		if (funct7NoBit0 == 0b0000000 || funct7NoBit0 == 0b0000100 // FADD.{S,D}, FSUB.{S,D}
			...
			return true;
		} else if (funct7NoBit0 == 0b1010000 && rm < 0b011) {
			emitFaultOnFSOff(addr);
			...
			if (rm == 0b000) // FLE.{S,D}
				emit8(0x02); // vcmples(s,d)
			else if (rm == 0b001) // FLT.{S,D}
				emit8(0x01); // vcmplts(s,d)
			else if (rm == 0b010) // FEQ.{S,D}
				emit8(0x00); // vcmpeqs(s,d)
